...
4. Commit with a Conventional Commits subject: `chore(release): X.Y.Z`
5. Annotated tag: `git tag -a vX.Y.Z -m "Release X.Y.Z"`
6. `git push origin master && git push origin vX.Y.Z` (a bare `git push` does not send tags)

The deploy workflow refuses to publish unless the tag is strict semver, matches `package.json`, and is an ancestor of `master`. Prerelease tags (`vX.Y.Z-rc.1`) do not trigger it at all.

**Do not run `npm install` just to sync the lockfile version.** The committed `package-lock.json` is generated by npm 11, which writes `libc` fields that older npm strips, producing large spurious diffs. Edit the two top-level `version` fields directly.
